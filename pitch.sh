...
install_dependencies() {
    ...
    sudo apt-get update
    sudo apt-get install -y bc build-essential zip curl libstdc++6 git wget python3 sshpass lvm2 libssl-dev rsync
...
sync_pbrp_source() {
    ...
    # Initialize and Sync
    ~/bin/repo init -u https://github.com/PitchBlackRecoveryProject/manifest_pb -b android-12.1 --depth=1
    ~/bin/repo sync -c -j$(nproc --all) --force-sync --no-tags --optimized-fetch --prune
}

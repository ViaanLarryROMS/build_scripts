...
install_dependencies() {
    ...
    sudo pacman -Syu --needed --noconfirm \
        base-devel bc git curl wget zip unzip \
        python openssh lvm2 openssl rsync \
        libxml2 ncurses readline
}

# Function to sync PBRP source code
sync_pbrp_source() {
    echo "--- Syncing PBRP source (directory setup only) ---"
    mkdir -p "$BUILD_DIR"
    cd "$BUILD_DIR"
...
echo "Output: $BUILD_DIR/out/target/product/$DEVICE_CODENAME/"
